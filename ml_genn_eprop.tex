...
\begin{figure*}[t]
  ...
  Bars signify the mean and error bars the standard deviation for 5 training runs with different seeds. Results are labeled with the network architecture, where ``ALIF''/''LIF'' denotes the neuron type, the subsequent number the number of neurons in the layer and ``F''/``R'' indicate a feedforward or recurrent layer, respectively.}
