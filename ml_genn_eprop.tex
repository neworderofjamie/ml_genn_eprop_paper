...
In this paper we introduce a new model description API for mlGeNN and functionality to train SNNs from scratch.
...
Finally we explore the addition of sparse connectivity to our models and show that this can reduce training time by around $10\times$ while still resulting in competitive classifcation performance.
...
\begin{figure*}[t]
  ...
  Bars signify the mean and error bars the standard deviation of 5 models trained with different seeds.\todo{describe notation}}
...
compiler = EPropCompiler(
    ...
    losses="sparse_categorical_crossentropy",\textbf
...
Efficiency is one of the main reasons why we believe dedicated tools for spike-based ML such as mlGeNN are necessary so it is important to compare the performance -- both for inference and training -- of models.
...
\todo{training performance}
\todo{GPU inference bad performance sparsity}
...
Although e-prop requires requires time-driven updates of synaptic variables (rather than the event-driven where GeNN has a more significant algorithmic advantage) and these updates are more complex than those required for FPTT, mlGeNN provides over $3.5\times$ the training throughput.
