...
\todo{CONNECTION EXAMPLE MISSING?}
...
The results of this exploration are shown in figures~\ref{fig:sparse_accuracy}~and~\ref{fig:two_layer_sparse_accuracy} and, while increasing sparsity clearly impacts performance, sparse configurations of ALIF512R are still highly competitive.
In fact, the best performing ALIF512R configuration -- with \SI{5}{\percent} connectivity in the input connections and \SI{10}{\percent} connectivity in the recurrent connections -- still out-performs the two layer EGRU model~\citep{subramoney2022egru}, even though it only has over $60\times$ fewer parameters.
...
\begin{itemize}
    ...
    \item Show effect of sparsity on performance - slower sparse inference due to lack of parallelism in small model with relatively small batch size due to small testing set.
\end{itemize}
